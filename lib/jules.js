...
- Wrap inline code references in single backticks: \`variableName\`
- All code suggestions MUST be inside fenced code blocks with the correct language tag:
  \`\`\`python
  # suggested fix
  ...
  \`\`\`javascript
  // suggested fix
  const result = document.getElementById("demo").textContent = "changed";
  \`\`\`
- Use \\n for newlines within the JSON string value
- Explain the "why" before showing the fix
      ...
      if (parsed.issues && Array.isArray(parsed.issues)) {
        for (const issue of parsed.issues) {
          if (validFiles.has(issue.file) && issue.line && issue.comment) {
            const category = issue.category || issue.severity?.toUpperCase() || "NOTE";
            ...
            comments.push({
              ...
              line: issue.line,
              side: "RIGHT",
              body: `**[${category}]** (${severity}): ${issue.comment}`,
            });
          }
        }
      }
