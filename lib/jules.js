...
// ─── Code block repair constants (compiled once) ─────────
const CODE_BLOCK_LANGS =
	"python|javascript|typescript|mermaid|bash|json|yaml|html|css|sql|ruby|java|csharp|cpp|hcl|lua|go|rust";
const BARE_TAG_REGEX = new RegExp(
	`(^|\\n)(?!\`\`\`)(${CODE_BLOCK_LANGS})\\n([\\s\\S]*?)(?=\\n\\n|\\n(?:${CODE_BLOCK_LANGS})\\n|$)`,
	"gi",
);

export class JulesReviewer {
	constructor(apiKey, mode = "SPEED") {
...
2. EVERY code suggestion MUST use fenced code blocks. A fenced code block has THREE backticks + language on the OPENING line, and THREE backticks ALONE on the CLOSING line. Supported languages: python, javascript, typescript, mermaid, bash, json, yaml, diff, hcl, lua, go, rust.
...
${fileContext}`;
	...
	_fixCodeBlocks(text) {
		...
		// Uses pre-compiled BARE_TAG_REGEX (module-level constant)
		BARE_TAG_REGEX.lastIndex = 0; // Reset stateful regex
		let result = text.replace(BARE_TAG_REGEX, (match, prefix, lang, code) => {
			if (!code.trim()) return match;
			return `${prefix}\`\`\`${lang}\n${code}\n\`\`\``;
		});

		// Pattern 2: Odd number of fences means one is unclosed — append closing fence
